...
rep_3=function(case,rho,family, sigmasq_t, B, alpha,delta, n, p, penalty,tune,screen,iter){
  ...
  SNR=sqrt(var(predy)/sigmasq_t)
  data$y=Y
  predictors=1:p
  sis_select=T
  if(screen==TRUE){
  if(family=="gam"){
    model_ini=greedINIS(data,quant=0.95,gnum = n-1)
    #model_ini=adaptINIS(data,quant=0.95)
...
simu_3.PL = function(case,family, B,n,p,sigmasq_t, alpha,
  ...
  write.table(cbind(case=case,rho=rho,penalty=penalty, eva_cs,iter=iter,k=k), file =paste(r.names,"_cs.csv",sep=""), sep=",",
...
#simu_3.PL(3,"binomial", B,n,p, sigmasq_t,
         #alpha, K, cores, rho=0.5,penalty = "adlasso",tune = tune,screen = T,iter=iter)
#simu_3.PL(4,"gam", 400, n=200,p=1000,0.5,
         # alpha, K=100, cores, screen = T)

simu_3.PL(4,"gam", B=400, n=200,p=30,1,alpha, K=100, cores, screen = F)
