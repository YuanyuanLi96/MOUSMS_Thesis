...
# 				}
          ...
          cr=tryCatch({if(restart_counter > control$restarts){
            stop("Sorry, but convergence could not be reached within the allocated number of
                 control$restarts. Consider increasing lambda[2].\n")}
            0}, error=function(e){cat("ERROR :",conditionMessage(e), "\n")
              return(1)})
     ...
     if(hybrid.est) {
          if(family$family[1] %in% c("ZIP","LOGNO")) { 
               ...
               if(all(sapply(1:length(id),function(k)length(out_list$nonzero.ranef[[k]])==0))){
                 final_fit <- suppressWarnings(glm(restring1,family = family,offset = offset, data = make_dat))
               }else{
               if(family$family[1] == "gaussian") 
